...
public struct AchievementNotificationView: View {
    ...
    public init(level: Int, levelName: String, stepRequirement: Int) {
        ...
        self.descriptionText = String(format: LocalizedKey.completedSteps.localized(), AchievementNotificationView.formatStepsStatic(stepRequirement))
    ...
    #else
    private func imageExists(_ name: String) -> Bool {
        return true // On macOS, assume images exist
    }
    #endif
...
//                        .frame(height: geometry.size.height * 0.08)
    ...
    #else
    private func captureAchievementImage() -> Any? {
        return nil
    }
    #endif
    ...
    private var achievementContent: some View {
        let imageName = badgeImageName ?? (level != nil ? "LV_\(level!)" : "LV_1")
        let currentTitleText = titleText
        let currentLevelName = levelName
        let currentDescriptionText = descriptionText
        let currentStepRequirement = stepRequirement
        let currentLevel = level
        let currentGradientStartColor = gradientStartColor
        
        return ZStack {
            LinearGradient(
                gradient: Gradient(colors: [
                    currentGradientStartColor,
                    Color(hex: "FFFFFF"),
                    Color(hex: "FFFFFF")
                ]),
                startPoint: .topTrailing,
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 24) {
                    if let titleText = currentTitleText {
                        Text(LocalizedKey.localizeAchievementTitle(titleText))
                            .font(.system(size: 60, weight: .bold))
                            .foregroundColor(.black)
                            .padding(.bottom, 8)
                    } else if let levelName = currentLevelName {
                        Text(LocalizedKey.localizeLevelName(levelName))
                            .font(.system(size: 60, weight: .bold))
                            .foregroundColor(.black)
                            .padding(.bottom, 8)
                    ...
                    if let descriptionText = currentDescriptionText {
                        Text(descriptionText)
                            .font(.system(size: 36))
                            .foregroundColor(.gray)
                    } else if let stepRequirement = currentStepRequirement {
                        Text(String(format: LocalizedKey.completedSteps.localized(), formatSteps(stepRequirement)))
                            .font(.system(size: 36))
                            .foregroundColor(.gray)
                    }
